...
# Define a hook function to capture the 'BLLH' activations
def capture_activations(layer_number, module, input, output):
    # output is expected to be a tuple where the first element is the attention output
    attention_output = output[0]
    # Store the 'BLLH' tensor for each head
    attention_activations[layer_number] = attention_output
...
def opt_eval(model, testenc, dev, force_train_as_val=False, dataset="ptb"):
    ...
    class Catcher(nn.Module):
        ...
        def forward(self, inp, **kwargs):
            ...
            raise ValueError
    layers[0] = Catcher(layers[0])
    # For each sample in testenc, store it appropriately in inps as a sample
    ...
    print(ppl.item())
    # # Detach the hooks to avoid memory leak
    # for layer in model.model.decoder.layers:
    #     layer.module.self_attn._forward_hooks.clear()
