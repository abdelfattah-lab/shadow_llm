...
class ParallelSP(nn.Module):
    ...
    def __init__(
        ...
        device=None,
        dtype=None,
        replace_ffn_predictor=False,
        skip_ffn_predictor=False,
        num_hidden_layers=1,
    ) -> None:
        super().__init__()
        assert (
        ...
        if replace_ffn_predictor and layer_idx == 0:
        ...
        self.replace_ffn_predictor = replace_ffn_predictor
        self.skip_ffn_predictor = skip_ffn_predictor

    def forward(self, x):
        if self.skip_ffn_predictor:
            squeezed_x = torch.squeeze(x)
            if squeezed_x.shape[0] < self.out:
                return F.pad(squeezed_x, (0, self.out - squeezed_x.shape[0]))
            else:
                return squeezed_x[:self.out]
        if self.replace_ffn_predictor:
            if self.layer_idx == 0:
...
class ParallelFusedMLPDejavu(nn.Module):
    def __init__(
        ...
        self.skip_ffn_predictor = sp_kwargs["skip_ffn_predictor"]
        self.replace_ffn_predictor = sp_kwargs["replace_ffn_predictor"]

        self.need_predictor = not(self.skip_ffn_predictor) and (layer_idx == 0 or not(self.replace_ffn_predictor))
        ...
        self.mlp_k = sp_kwargs["K"] // self.process_group.size()
    ...
    def forward(self, x, residual, mlp_logit=None, idx=None):
        if self.heuristic == "auto":
            dtype = (
                x.dtype
                if not torch.is_autocast_enabled()
        ...
        if do_token_generation and self.layer_idx != 0 and self.skip_ffn_predictor:
            # Set idx to random self.mlp_k numbers from 0 to self.fc1.weight.shape[0]
            idx = torch.randint(0, self.fc1.weight.shape[0], (self.mlp_k,), device=x.device)
        ...
        if self.need_predictor:
            with torch.cuda.stream(self.sp_stream):
                ...
                if do_token_generation:
                    ...
                    # Include predictions only when in the mode of replacing the ffn predictor.
                    if self.replace_ffn_predictor:
                        mlp_predictions = mlp_logit
                self.sp_stream.record_event(self.event_mlp_sp)

            curr_stream.wait_event(self.event_mlp_sp)

        return out, mlp_logit, mlp_predictions
