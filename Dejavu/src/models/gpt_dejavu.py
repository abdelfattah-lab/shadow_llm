...
def create_mixer_cls(
    ...
    if att_sparse:
        ...
        sp_kwargs = {
            ...
            "K": att_k,
            "replace_att_predictor": config.replace_att_predictor,
            "skip_att_predictor": config.skip_att_predictor,
    ...
    if fused_mlp:
        ...
        if mlp_sparse:
            ...
            sp_kwargs = {
                ...
                "K": mlp_k,
                "replace_ffn_predictor": config.replace_ffn_predictor,
                "skip_ffn_predictor": config.skip_ffn_predictor,
                "num_hidden_layers": config.num_hidden_layers,
            }
...
class GPTModel(GPTPreTrainedModel):
    def __init__(self, config: GPT2Config, process_group=None, device=None, dtype=None):
        ...
        # pdb.set_trace()
        # # The first layer which will have the predictor inside
        # self.layers = nn.ModuleList(
        #     [
        #         create_block(
        #             config, layer_idx=0, process_group=process_group, **factory_kwargs
        #         )
        #     ]
        # )
        ...
        if self.sparse:
            ...
            for layer in self.layers:
                ...
                if mlp_predictions is not None:
                    layer_mlp_predictions = mlp_predictions[(layer.layer_idx-1) * layer.mlp.fc1.out_features: (layer.layer_idx) * layer.mlp.fc1.out_features]
                else:
                    layer_mlp_predictions = None
                if att_predictions is not None:
                    layer_att_predictions = att_predictions[layer.layer_idx * layer.mixer.num_heads // self.process_group.size(): (layer.layer_idx + 1) * layer.mixer.num_heads // self.process_group.size()]
                else:
                    layer_att_predictions = None
                if self.prenorm:
                    (
                        hidden_states,
                        residual,
                        ...
                        hidden_states,
                        residual,
                        att_predictions=layer_att_predictions,
                        mlp_predictions=layer_mlp_predictions,
                        head_idx=next_att_idx,
                        mlp_sp_logit=next_mlp_sp_logit,
                        mixer_kwargs=mixer_kwargs,
                    )
                    if n_mlp_predictions is not None:
                        mlp_predictions = n_mlp_predictions
                    if n_att_predictions is not None:
                        att_predictions = n_att_predictions
                    i += 1
                else:
